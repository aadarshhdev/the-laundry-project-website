...
const TIMES = ['8:00 AM – 10:00 AM', '10:00 AM – 12:00 PM', '12:00 PM – 2:00 PM', '2:00 PM – 4:00 PM', '4:00 PM – 6:00 PM', '6:00 PM – 8:00 PM'];
const TODAY = new Date().toISOString().split('T')[0];

const inputCls = 'w-full border border-gray-200 rounded-xl text-sm text-secondary placeholder-gray-400 focus:outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-100 transition-all bg-white';

export default function BookingModal({ open, onClose }) {
  const [form, setForm] = useState({ name: '', phone: '', address: '', service: '', date: '', time: '' });
                ...
                {submitted ? (
                  ...
                  >
                    ...
                    <p className="text-secondary-600 text-sm leading-relaxed mb-8 max-w-xs mx-auto">
                      We've received your booking request. Our team will call you within 30 minutes to confirm the pickup slot.
                    </p>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <input name="name" value={form.name} onChange={handleChange} required placeholder="Your name" className={`${inputCls} pl-9 pr-3 py-2.5`} />
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <input name="phone" value={form.phone} onChange={handleChange} required placeholder="+91 XXXXX" className={`${inputCls} pl-9 pr-3 py-2.5`} />
                        </div>
                      </div>
                    </div>

                    <div>
                      <label className="block text-xs font-semibold text-secondary-600 uppercase tracking-wide mb-1.5">Service</label>
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-secondary-600 uppercase tracking-wide mb-1.5">Pickup Address</label>
                      <div className="relative">
                        <FiMapPin className="absolute left-3 top-3 w-4 h-4 text-secondary-400" />
                        <textarea name="address" value={form.address} onChange={handleChange} required rows={2} placeholder="Enter your full address" className={`${inputCls} pl-9 pr-3 py-2.5 resize-none`} />
                      </div>
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-semibold text-secondary-600 uppercase tracking-wide mb-1.5">Pickup Date</label>
                        <div className="relative">
                          <FiCalendar className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-secondary-400" />
                          <input type="date" name="date" value={form.date} onChange={handleChange} required min={TODAY} className={`${inputCls} pl-9 pr-3 py-2.5`} />
                        </div>
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-secondary-600 uppercase tracking-wide mb-1.5">Time Slot</label>
